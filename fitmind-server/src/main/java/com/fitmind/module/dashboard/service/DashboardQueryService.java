package com.fitmind.module.dashboard.service;
...
public class DashboardQueryService {
    ...
    @Cacheable(cacheNames = CacheNames.DASHBOARD_STATS, key = "#userId")
    ...
    @Cacheable(cacheNames = CacheNames.DASHBOARD_WEEKLY_TRAINING, key = "#userId")
    public List<Map<String, Object>> getWeeklyTraining(Long userId) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            int totalMinutes = weekPlans.stream()
                    .filter(p -> currentDate.equals(p.getPlanDate()))
    ...
    @Cacheable(cacheNames = CacheNames.DASHBOARD_HEATMAP, key = "#userId")
    public List<List<Map<String, Object>>> getHeatmap(Long userId) {
    ...
    @Cacheable(cacheNames = CacheNames.DASHBOARD_NUTRITION_TODAY, key = "#userId")
    public Map<String, Object> getNutritionToday(Long userId) {
    ...
    @Cacheable(cacheNames = CacheNames.DASHBOARD_BODY_METRICS_TREND, key = "#userId")
    public List<Map<String, Object>> getBodyMetricsTrend(Long userId) {
    ...
    private int calculateStreak(List<AiTrainingPlan> training, List<AiDietPlan> diet) {
        ...
        if (completedDates.isEmpty() && activeDates.isEmpty()) {
            return 0;
        }
